# RAG入门课
根据飞书讲义整理：认知阶段、提示词、RAG整体认知、Embedding、向量数据库、Native RAG、Advanced RAG、检索前优化（Pre-retrieval）、检索中优化（Retrieval）。
...
目标：提高精排质量与生成可用性——捞上来的材料怎么用。召回靠第 09 章抬上来，这里负责精排与怎么喂给模型。
...
#### 第 09 章：把「检索中」拆开练：混合检索 / 多路召回 / RRF + 代码落点
...
##### 5 喂给大模型 —— 可再接第 07 章重排与压缩

#### 和本仓库

##### 当前 chroma文档管理 ≈ 单路稠密 Native

##### 最小改法：同 nodes 加 BM25 + QueryFusionRetriever(mode=reciprocal_rerank)

##### 有多目录知识时再拆多路并打 channel 元数据

# RAG入门课
根据5篇飞书讲义整理：认知阶段、提示词、RAG整体认知、Embedding、向量数据库。
...
###### LLM 专注自然语言：理解、生成、翻译、推理文本

###### LLM 是大模型家族里最成熟、应用最广的分支

###### 大模型已发展为多模态，不止语言

###### 所有大语言模型都是大模型，但并非所有大模型都是大语言模型
...
#### 设计原则：明确、完整、角色风格一致、考虑安全

#### 调优策略：先零样本再加复杂度、按输出迭代、建立评估标准、记录有效模板

#### 趋势：自动生成优化提示、多模态提示、按反馈动态调、按用户特征个性化
...
#### 1.2 和纯大模型的区别：不是替代，而是装上实时、可信、可控的记忆外挂
...
##### 检索质量决定上限：最相关文件没召回，生成再强也没用

##### 上下文窗口：答案分散在多处时可能被截断

##### 检索噪声：无关片段会误导模型

##### 延迟增加：比纯 LLM 多一步检索，需要异步或缓存

##### 依赖 Embedding 和切分策略，中文和专业术语很敏感

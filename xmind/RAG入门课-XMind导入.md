...
核心：向量库还没开始算相似度之前，把「问句」和「文档形态」准备好。下面每项都按：在干什么 → 怎么操作 → 注意点。

### 〇、先搞清边界

#### 检索前在干什么

##### 对用户问题：改写、扩展、拆分、澄清、清洗术语

##### 对知识库（多在离线）：分块、打标签、建多表示、路由规则

##### 目标：问得清楚、搜得准范围、少噪声

#### 和后面两步的分界

##### 检索中：真正去 Chroma/FAISS/BM25 里搜

##### 检索后：对搜回来的结果重排、压缩、再生成

##### 记住：问句歪了，后面 rerank 也很难救

### 一、查询重写 Query Rewriting

#### 在干什么

##### 把口语、指代不明、缺词的问句，改成检索友好的完整表达

##### 例：上次那个产品的安全规范 → XX产品 最新版 安全规范 文档

##### 不改变用户意图，只改变「怎么说」

#### 怎么操作（手工/Prompt）

##### 1 写改写 Prompt：你是检索改写助手，只输出改写后的查询，不要解释

##### 2 要求：补全实体名、去掉口语、保留原意、可加同义术语

##### 3 temperature 调低（如 0~0.3），减少乱跑

##### 4 用改写后的句子去做 embedding + 检索

##### 5 可选：原句和改写句都检索，结果合并（防改歪）

#### 怎么操作（LlamaIndex）

##### 自定义类继承 BaseQueryTransform，在 _run 里调 LLM 改写

##### 或用现成 TransformQueryEngine 包一层

##### engine.query(原问题) 时内部先改写再检索

#### 注意

##### 改写过头会偏题 → 保留原查询一起搜

##### 适合：指代、口语、缺关键词；不适合：已经很规范的短事实问句（可跳过）

### 二、查询扩展 Query Expansion / Multi-Query

#### 在干什么

##### 同一意图生成多个近义/不同句式变体，多路检索再合并

##### 例：请假怎么扣钱 → 事假扣款；年假规则；考勤制度旷工；迟到罚款

##### 目的：提高召回，怕用户用词和文档用词对不上

#### 怎么操作

##### 1 Prompt：请为下列问题生成 N 个检索用变体，每行一个，不要编号解释

##### 2 对每个变体分别做向量检索，各取 Top-K

##### 3 用 RRF 或去重合并：同一文档多次命中应加分或只留一次

##### 4 再把合并后的 Top-M 交给生成（或先重排）

#### LlamaIndex 操作

##### QueryFusionRetriever(retrievers=[...], num_queries=N, mode='reciprocal_rerank')

##### 它会自动生成多查询、多路 retrieve、按排名融合

#### 注意

##### N 太大：延迟和费用上去，噪声也多；一般 3~5 个变体

##### 和「分解」不同：扩展是同义多说法，不是拆成不同子问题

### 三、HyDE 假设文档检索

#### 在干什么

##### 先让 LLM 写一篇「假想的理想答案文档」，再用这篇去向量库搜

##### 原理：库里存的是「答案体」段落，假想答案和它更像，短问句不像

##### 例：问「年假怎么算」→ 先生成一段像制度条文的假想说明 → 用这段去搜真制度

#### 怎么操作（逐步）

##### 1 调 LLM：请写一段可能回答该问题的文档片段（不要对话口吻）

##### 2 对假想文档做 embedding（用和建库同一模型）

##### 3 在向量库里 search，取 Top-K

##### 4 强烈建议：同时用「原问题」再搜一路，两路结果合并

##### 5 把合并后的真实文档片段交给生成模型回答（不要把假想文档当事实）

#### LlamaIndex 操作

##### hyde = HyDEQueryTransform(include_original=True)

##### engine = TransformQueryEngine(base_query_engine, hyde)

##### engine.query(用户问题) 即可；include_original=True 就是第 4 步

#### 注意

##### 假想写偏会带偏检索 → 必须保留原查询或人工抽查

##### 多一次 LLM，延迟和费用上升

##### 适合：问句短、表述和文档差大；不适合：极严事实且模型完全不懂该领域

### 四、子查询分解 Query Decomposition

#### 在干什么

##### 把一个复杂问题拆成多个原子子问题，分别检索（或分别回答），最后综合

##### 例：比较 A/B 公司 2023 营收增长

##### 子问1：A 2023 营收；子问2：B 2023 营收；子问3：各自相对上年增长率；最后对比

#### 怎么操作

##### 1 LLM 分解：请把问题拆成可独立检索的子问题列表（JSON）

##### 2 对每个子问题调用同一个或不同的 query_engine

##### 3 收集各子答案/证据片段

##### 4 再调 LLM：根据下列证据综合回答原问题，并标注来源

#### LlamaIndex 操作

##### 把各知识库包成 QueryEngineTool，description 写清「这个工具查什么」

##### SubQuestionQueryEngine.from_defaults(query_engine_tools=tools)

##### engine.query(复杂问题) 会自动拆、路由、并发、综合

##### description 糊了会路由错库，这是最常见翻车点

#### 注意

##### 适合比较题、多跳题、跨文档题；不适合一句闲聊

##### 和扩展区别：分解是不同信息点；扩展是同一意思多种说法

### 五、Step-Back 后退提问

#### 在干什么

##### 先退一步问更宽泛的背景/原理问题，检索背景后再回答原细节问题

##### 例：Qwen2.5-7B 上下文多长 → 先检索「主流 LLM 上下文窗口一般多少」再答具体型号

#### 怎么操作

##### 1 Prompt：把具体问题改写成一个更抽象的背景问题

##### 2 用背景问题检索，拿到通识/原理片段

##### 3 再用原问题检索细节（或直接带着背景片段生成）
...
##### 问题含糊时先问清意图，再检索；避免乱搜一堆无关制度

##### 例：「请假扣钱吗」→ 先问：事假、病假还是年假？

#### 怎么操作

##### 1 用小模型/规则判断：是否缺实体、是否多意图、是否指代不明

##### 2 若模糊：返回澄清问题给用户，本轮不检索或只做轻量检索

##### 3 用户补充后，把「澄清后的完整问题」再走改写/检索

##### 4 可做分支：意图A走制度库，意图B走 FAQ 库

### 七、查询向量化前的文本清洗（很便宜）

#### 在干什么

##### 去掉口语废话，统一术语，抽出实体，让 embedding 更干净

#### 怎么操作

##### 1 去口语：删掉「那个」「嗯」「帮我看看」等无信息词

##### 2 术语表映射：年假=带薪年休假；电脑=笔记本电脑（按你们库用词）

##### 3 实体抽出：产品名、工号、日期，可同时丢给 BM25

##### 4 再 embedding；专名多时建议混合检索

### 八、文档分块优化（离线，但是检索前）

#### 在干什么

##### 决定「一段知识怎么切成可检索的块」：切得好，后面才搜得着、答得全

##### 切太碎：命中准但上下文残缺；切太大：向量稀释检不中

#### 怎么操作：SentenceSplitter（课上默认）

##### 1 splitter = SentenceSplitter(chunk_size=512, chunk_overlap=100)

##### 2 nodes = splitter.get_nodes_from_documents(docs)

##### 3 入库 index.insert_nodes(nodes)

##### 4 overlap=100：相邻块重叠，防止关键句被切断

##### 5 中文可配 secondary_chunking_regex，按。！？切

#### 怎么操作：语义分块

##### 1 先中文分句（。！？换行）

##### 2 SemanticSplitterNodeParser(buffer_size=1, breakpoint_percentile_threshold=95, embed_model=...)

##### 3 算句间相似度，主题一变（相似度下跌）就切开

##### 4 更慢（要 embedding），适合长文质量优先

##### 5 空文本先 clean_empty_text，否则会报错

#### 怎么操作：父子块 Parent-Child

##### 1 HierarchicalNodeParser.from_defaults(chunk_sizes=[2048, 512])

##### 2 小块（叶子）建向量索引；大块（父）放 docstore

##### 3 检索时用 AutoMergingRetriever：小块命中后合并回父块

##### 4 效果：检索准（小块）+ 生成有上下文（大块）

#### 参数怎么调（实操）

##### 先 512/100 跑通，看坏例：答案缺上下文就加大块或加 overlap
...
##### 给每块打上 title/source/date/category/page 等标签

##### 后面可以 where 过滤、展示引用、按年归档

#### 怎么操作：打标签 + 预过滤

##### 1 入库时 metadata={'category':'年假','year':2024,'source':'考勤手册'}

##### 2 查询时先解析用户意图（只要 2024、只要年假）

##### 3 Chroma：query(..., where={'category':'年假'}) 先过滤再向量搜

##### 4 口述：先缩小书架，再在书架里找相似段落

#### 意图路由怎么操作

##### 1 准备多个索引/集合：制度库、FAQ 库、技术文档库

##### 2 用分类器或 LLM 判断问题类型

##### 3 只去对应库检索；或拆成工具 description 交给 SubQuestion 路由

##### 4 description 必须写清差异，否则会路由乱

#### 权限过滤怎么操作

##### 1 文档 metadata 带 allowed_roles / dept

##### 2 检索前 where 加上当前用户角色条件

##### 3 绝不能：先搜出敏感段再靠 Prompt 说「别泄露」

### 十、文档增强与多表示索引（离线）

#### 在干什么

##### 除了正文，再存摘要、关键词、假设问题等，提高可检索性

##### 像给每本书写目录、标签、常见问答，找起来更容易

#### 怎么操作

##### 1 对每个 chunk 用 LLM 生成：摘要一句、3~5 关键词、2 个假设用户问题

##### 2 把假设问题也向量化入库（或与 chunk 绑同一 id）

##### 3 检索时可对「假设问题」匹配（类似反向 HyDE）

##### 4 或多路：摘要向量一路 + 正文向量一路 + 关键词 BM25 一路，再融合
...
##### 2 选分块：先 Sentence 512/100，难例再试语义/父子

##### 3 打 metadata（来源、分类、时间）

##### 4 同一 Embedding 写入 Chroma，记下模型名
...
##### 3 否则：改写 或 HyDE(include_original) 或 Multi-Query

##### 4 有类别/时间意图 → where 预过滤

##### 5 再进入向量检索（检索中）

#### 对照本仓库怎么加

##### 现有：三种分块 + /query（Native）

##### 最小改动：在 query() 里先 LLM 改写再 retrieve

##### 进阶：TransformQueryEngine + HyDEQueryTransform

##### 再进阶：Multi-Query / SubQuestion / 父子块

### 十二、选型速查（遇到问题选哪个）

#### 指代/口语 → 查询重写

#### 问句太短、和文档不像 → HyDE + 保留原句

#### 用词不准、召回低 → Multi-Query 扩展
